    ...
    processFieldRef(web, conf, fieldRef) {
        return new Promise((resolve, reject) => {
            web.lists.getByTitle(conf.Title).fields.getById(fieldRef.ID).update({ Required: fieldRef.Required, Title: fieldRef.DisplayName }).then(() => {
                sp_pnp_js_1.Logger.log({ data: fieldRef, level: sp_pnp_js_1.LogLevel.Info, message: `Field '${fieldRef.ID}' updated with Title '${fieldRef.DisplayName}' for list ${conf.Title}.` });
                resolve();
            }, reject);
        });
    }
