...
class Lists extends handlerbase_1.HandlerBase {
                    ...
                    if (removeExisting) {
                        ...
                        list.contentTypes.get().then(contentTypes => {
                            contentTypes.forEach(({ Id: { StringValue } }) => {
                                ...
                                if (shouldRemove) {
                                    sp_pnp_js_1.Logger.write(`Removing content type ${StringValue} from list ${conf.Title}`, sp_pnp_js_1.LogLevel.Info);
    ...
    processField(web, conf, fieldXml) {
        return web.lists.getByTitle(conf.Title).fields.createFieldAsXml(this.replaceFieldXmlTokens(fieldXml)).then(({ data }) => {
            sp_pnp_js_1.Logger.log({ data: data, level: sp_pnp_js_1.LogLevel.Info, message: `Field added successfully to list ${conf.Title}.` });
        });
    }
    /**
