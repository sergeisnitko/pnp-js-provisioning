...
class ComposedLook extends handlerbase_1.HandlerBase {
    ...
    ProvisionObjects(web, composedLook) {
        ...
        return new Promise((resolve, reject) => {
            web.applyTheme(util_1.ReplaceTokens(composedLook.ColorPaletteUrl), util_1.ReplaceTokens(composedLook.FontSchemeUrl), composedLook.BackgroundImageUrl ? util_1.ReplaceTokens(composedLook.BackgroundImageUrl) : null, false).then(_ => {
                super.scope_ended();
                resolve();
            }).catch(e => {
                super.scope_ended();
